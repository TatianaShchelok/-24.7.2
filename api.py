...
class PetFriends:
 ...
 def get_api_key(self, email: str, password: str) -> json:
    ...
    def post_add_a_new_pet(self, auth_key: json, name: str, animal_type: str, age: str, pet_photo: str) -> json:
        """
        Метод отправляет(постит) на сервер данные о добавляемом питомце и возвращает статус запроса  и
    ...
    def create_pet_simple(self, auth_key: json, name: str, animal_type: str, age: str):
        """Метод отправляет на сервер запрос о создании нового питомца и возвращает статус
        запроса и результат в формате JSON с данными созданного питомца"""
    ...
    def set_photo(self, auth_key: json,  pet_photo: str, pet_id= str) -> json:
        """Метод позволяет добавить фото в уже существующую карточку питомца"""
        file = {'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')}
        headers = {'auth_key': auth_key['key'], 'pet_id': pet_id}
        res = requests.post(self.base_url + 'api/pets/set_photo', headers=headers, files=file)
        status = res.status_code
        try:
            result = res.json()
        except json.decoder.JSONDecodeError:
